...
class Debits extends Component {
    ...
    submitDebit(event) {
        ...
        let tempDebit = [event.target.debitDes.value, event.target.debitAmount.value, new Date().toLocaleString()]
  ...
  render() {
    return (
        <div>
            ...
            <div>
                <div>
                    <h4>Debits: ${(Math.round(this.props.debitBalance* 100) / 100).toFixed(2)}</h4>

                    <form onSumbit={this.submitDebit}>
                        <input type="text" name="debitDes" placeholder="Debit Description"/>
